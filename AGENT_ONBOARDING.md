...
### 2026-09-24 — printed forms in the approved design, company profile
- **Design (owner's reference PDFs):** monochrome A4, faint wave lines behind the page, the title in a frame with corner brackets and a subtitle in brackets, a meta line (number, dates, method badge), two party blocks, tables with a heavy top rule and zebra rows, totals / cards, notes, terms beside a recipient box, signature boxes (and a dashed stamp box), and a footer with contacts, bank details and QR codes. Fonts embedded: **Tajawal** (Arabic, Light→ExtraBold), **IBM Plex Mono** (figures, codes, e-mails), Noto Sans fallback — OFL licences next to them in `Infrastructure/Exports/Fonts`.
- **Engine:** still QuestPDF on the server. `Exports/PdfLayout.cs` composes every page; `Exports/PdfArt.cs` holds the SVG pieces (waves, corners, rounded / dashed boxes sized to their content via `Svg(size => …)`, Lucide icons, stamp). `ExportDocument` gained an optional `ExportLayout` (meta, parties, opening bar, receipt body, summary + total, cards, note, terms, recipient box, signatures, stamp, payment details, QR) and `ExportTable` optional money / code / positive / shaded columns and widths — every existing caller is unchanged and simply gets the new letterhead. Figures are sized to their column (`Fit`) so a number never splits; with six-figure amounts the currency moves to the column header; negatives print as (32.00) (a leading minus flips in Arabic).
- **Official documents are composed on the server from the database** (`Features/Printing/OfficialDocuments.cs`: pure builders `InvoicePrint`, `ReceiptPrint`, `StatementPrint` + handlers): `GET /invoices/{id}/pdf` (sale / return / credit note), `GET /payments/{id}/pdf` (receipt voucher, "سند صرف" for refunds), `GET /customers/{id}/statement/pdf`. Currency rule: invoice in pounds when recorded (dollar equivalent beside the total), receipt in the currency received (words in that currency, `PrintFormat.SypInWords` / `UsdInWords`), statement in **dollars** (receipts paid in dollars carry no pound amount, so only the dollar ledger is complete).
- **Company profile** (migration 30 `company_profile`, single row, kept by the reset): name, manager, address, contacts, WhatsApp (QR "راسلنا واتساب"), bank, IBAN, invoice subtitle / terms, receipt note, statement note. `GET/PUT /api/v1/company-profile` (read: any signed-in user; write: `system:config_write`), screen **بيانات المنشأة** (`/company`, with a letterhead preview). QR links use `IAppLinks` (`App:PublicUrl`, else the first `AllowedOrigins`).
- **Browser:** `DocumentDialog` shows the PDF itself (iframe) where the browser has a PDF viewer, else the data table; buttons print / open in a tab / PDF / Excel / CSV. `exportClient`: `PdfSource` (`exportPdf(doc)`, `endpointPdf(request)`), `printPdf` (hidden frame on desktop Chrome/Edge/Firefox; Safari and phones open a tab, opened at the click so it is not blocked; a frame that cannot print falls back to the tab), `downloadPdf`, `previewPdfFrom`. Invoice page: "عرض / طباعة"; receipts and the customer statement pass their server PDF to the viewer.
- **Verified:** `PrintedFormsTests` render the three forms with the reference content (one page each) and a plain list (set `PRINT_PREVIEW_DIR` to get PNGs); `print_test.js` against the live API (migration, profile read / update / validation, the three PDFs from real rows incl. a return and a draft, 404 for an unknown receipt, generic and 60-row landscape exports, Excel). Unit 225, integration 35, tsc, build. Not clicked through in a signed-in browser (no password typing).
