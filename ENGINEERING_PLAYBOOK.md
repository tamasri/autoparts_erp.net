...
- **Styling:** **MUI v6 `createTheme`** with `direction: 'rtl'` as the single source of truth. The design tokens (the identity colours in `brand`, chart colours, `12px` radii, card shadows, font stack) and the pattern (`theme/pattern.ts`: `patternImage`, `patternSx` — keep it at low opacity) and the few global rules (`MuiCssBaseline`) live in `theme.ts`; there is no CSS file and no class-name styling. Build screens only from MUI and `components/ui` (PageHeader, DataTable, StatusChip, ReasonDialog, RoutedTabs, Money, KpiTile). **No new colour literals** — use theme palette or CSS variables from `theme.css` during migration.
...
- **Session:** the access token is in memory (`stores/authStore`, not persisted); the refresh token is the HttpOnly `erp_rt` cookie, set and read only by `AuthModule` (`RefreshCookie`). Never return a refresh token in JSON or store a token in localStorage. The API client retries a 401 once after `refreshSession()` (`lib/session.ts`: one refresh at a time per tab and across tabs via Web Locks, because refresh tokens are single-use). Any new `fetch`/`axios` call outside `api/client.ts` must send `X-Requested-With` and handle 401 the same way.
- **ERPNext protocol (2026-09-24):** ERPNext is Frappe v16 in production and v16 changed the REST query rules. Aggregate columns are `ErpNextClient.Aggregate` values (never SQL text); the client sends the v16 dict form and falls back once to the v≤15 text form, remembering the answer per server. Every failed call goes through `Explain` → `ErpNextErrors`: the short Frappe message is returned (sync log, screens), the full body (traceback) is logged on the server only.
- **Parties in ERPNext are identified by record, never by display name (2026-09-24).** Display names are not unique here and ERPNext may name records by series (CUST-00001). Every hand-off links a customer/supplier with `ErpNextPartyLinks.EnsureAsync` and uses the returned record name in the document. A rename updates `customer_name`/`supplier_name` on the same record (no `rename_doc`). Two local parties never share a record: a same-named second one is created as "Name (PARTY-CODE)".
- **Lookups keyed by external names must tolerate repeats:** choose one row in SQL (`DISTINCT ON (erpnext_name) … ORDER BY (status = 'SYNCED') DESC, updated_at DESC`), never `ToDictionary` over names that come from ERPNext.
- **The ERPNext mock must behave like the real server:** `FRAPPE_VERSION` (16 default, 15), `PARTY_NAMING` (name/series), Frappe's error body, 404 for unknown records, link validation on documents. A mock that accepts everything hid the v16 break.
- **Seeding:** `DatabaseSeeder` (roles, first admin) and `ReferenceDataSeeder` (reason codes, KPI definitions) run everywhere; `DemoDataSeeder` only in Development or with `Seed:DemoData=true`. Check a new migration or seeder on an EMPTY database, not only on the dev one.
- **Printing:** every PDF goes through `DocumentRenderer` / `PdfLayout` (QuestPDF). A new printed document is an `ExportDocument` (plus an `ExportLayout` for an official form) — no other PDF code. Official documents (anything with legal weight: invoices, vouchers, statements) are composed on the server from the database by a pure builder in `Features/Printing`, never from what the browser sends. In a PDF, keep a number and Arabic text in separate elements when their order matters (bidi can reorder "123-456" inside an Arabic paragraph), print negatives in brackets, and size figures with `Fit` instead of letting them wrap. Browser side: print with `printPdf`, never `window.print()` on the page.
- **Money rounding:** round amounts that are paid out (commission, shares) with `MidpointRounding.AwayFromZero`; .NET's default rounds half to even.
- **PL/pgSQL:** wrap a CASE inside an IF condition in brackets — `IF x <> (CASE … THEN … END) THEN`; otherwise the IF ends at the CASE's own THEN ("syntax error at end of input").
- **Costs and returns:** any movement with a known unit cost goes through `InventoryCosting` (before the stock moves). A return is created from its original only (`…/returns`), never as a free-standing invoice; balances are generated columns — change `paid_*` / `credit_applied_*`, never a balance.
- **Numbered documents:** never generate a number in code. Insert without it and read it back with `RETURNING <number column>`; the database numbers the row (`document_series`). A new numbered table goes in `AddDocumentNumbering.NumberedTables` style (series, triggers) **and** `NumberedDocuments.All` (a unit test keeps the two equal). Documents are never deleted by code paths other than `DeleteDocumentCommand` (the database refuses a delete without its `deleted_documents` row).
- **New table?** Add it to `BusinessDataReset.Wiped` (business data) or `.Kept` (installation/reference data); the reset tool refuses to run while a table is in neither list.
- **Dapper and arrays:** a `uuid[]`/`text[]` column cannot be a positional record constructor parameter (materialization fails at run time, not compile time) — read it with `ExecuteScalarAsync<Guid[]>` or into a class with settable properties.
- **Real-time:** send through `IRealtimeNotifier` to a permission (`RealtimeGroups.NotifiedPermissions` must list it) or to user ids, after the change is committed; never let a failed send fail the action. Clients never pick groups.
...
| **Frontend React** | `frontend/src/*` | screens for existing endpoints, `usePagedList`, MUI + `components/ui`, RTL Arabic | call the API outside the shared client, add a second state/data library, aggregate in the browser |
